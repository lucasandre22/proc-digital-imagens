...
/*void merge(Imagem* img, Imagem* bordas, Imagem* bordas_falsas, SHAPES* shapes) {
    float pesos[2][5][5];
    Imagem* final = criaImagem(img->largura, img->altura, CANAIS);
    ...
    for(int h = 0; h < shapes->quantidade; h++) {
        ...
        if(tamanhoi == tamanhoj) { //quando é um só píxel ou tem 2 de altura e 2 de largura, tratamento diferente
            ...
            if(vetor[shapes->l_shapes[h].borda_falsa]->dados[0][corneri][cornerj+1] == PONTA_FALSA) {
                final->dados[0][corneri][cornerj] = ((pesos[tamanho_menor][tamanho_maior][0]) * img->dados[0][corneri][cornerj+1]) +
                ...
                final->dados[0][corneri][cornerj] = ((pesos[tamanho_menor][tamanho_maior][0]) * img->dados[0][corneri-1][cornerj]) +
                                                ((1-pesos[tamanho_menor][tamanho_maior][0]) * img->dados[0][corneri][cornerj]);
            }
            else if(vetor[shapes->l_shapes[h].borda_falsa]->dados[0][corneri+1][cornerj] == PONTA_FALSA) {
                final->dados[0][corneri][cornerj] = ((pesos[tamanho_menor][tamanho_maior][0]) * img->dados[0][corneri+1][cornerj]) +
        ...
        if(maior) { //I É MAIOR
            ...
            if(inegativo) { //i vai negativando
                for(int i = corneri, f = 0; i > shapes->l_shapes[h].iFinal; i--, f++) { //f controla peso
                    if(pesos[tamanho_menor][tamanho_maior][f] != 0) {
                        aux = img->dados[0][i][cornerj];
                        final->dados[0][i][cornerj] = (pesos[tamanho_menor][tamanho_maior][f] * img->dados[0][i][coordenada_pixel_misturar]) 
            ...
            if(jnegativo) {
                for(int j = cornerj, f = 0; j > shapes->l_shapes[h].jFinal; j--, f++) { //f controla peso
                    if(pesos[tamanho_menor][tamanho_maior][f] != 0) {
                        aux = img->dados[0][corneri][j];
                        final->dados[0][corneri][j] = (pesos[tamanho_menor][tamanho_maior][f] * img->dados[0][coordenada_pixel_misturar][j]) 
...
void merge(Imagem* img, Imagem* bordas, Imagem* bordas_falsas, SHAPES* shapes) {
    ...
    for(int h = 0; h < shapes->quantidade; h++) {
        ...
        if(tamanhoi == tamanhoj) { //quando é um só píxel ou tem 2 de altura e 2 de largura, tratamento diferente
            aux = img->dados[0][corneri][cornerj];
            if(vetor[shapes->l_shapes[h].borda_falsa]->dados[0][corneri][cornerj+1] == PONTA_FALSA) {
                for(int p = 0; p < CANAIS_IMAGEM; p++)
                    final->dados[p][corneri][cornerj] = ((pesos[tamanho_menor][tamanho_maior][0]) * img->dados[p][corneri][cornerj+1]) +
                                                ((1-pesos[tamanho_menor][tamanho_maior][0]) * img->dados[p][corneri][cornerj]);
            }
            else if(vetor[shapes->l_shapes[h].borda_falsa]->dados[0][corneri][cornerj-1] == PONTA_FALSA) {
                for(int p = 0; p < CANAIS_IMAGEM; p++)
                final->dados[p][corneri][cornerj] = ((pesos[tamanho_menor][tamanho_maior][0]) * img->dados[p][corneri][cornerj-1]) +
                ...
                for(int p = 0; p < CANAIS_IMAGEM; p++)
                final->dados[p][corneri][cornerj] = ((pesos[tamanho_menor][tamanho_maior][0]) * img->dados[p][corneri-1][cornerj]) +
                                                ((1-pesos[tamanho_menor][tamanho_maior][0]) * img->dados[p][corneri][cornerj]);
            }
            else if(vetor[shapes->l_shapes[h].borda_falsa]->dados[0][corneri+1][cornerj] == PONTA_FALSA) {
                for(int p = 0; p < CANAIS_IMAGEM; p++)
        ...
        if(maior) { //I É MAIOR
            ...
            if(inegativo) { //i vai negativando
                for(int i = corneri, f = 0; i > shapes->l_shapes[h].iFinal/*+ tamanhoi/2 */; i--, f++) { //f controla peso
                    if(pesos[tamanho_menor][tamanho_maior][f] != 0) {
                        for(int p = 0; p < CANAIS_IMAGEM; p++)
                        final->dados[p][i][cornerj] = (pesos[tamanho_menor][tamanho_maior][f] * img->dados[p][i][coordenada_pixel_misturar]) 
                        + ((1-pesos[tamanho_menor][tamanho_maior][f]) * img->dados[p][i][cornerj]);
                    }
                }
            }
            else {
            ...
            if(jnegativo) {
                for(int j = cornerj, f = 0; j > shapes->l_shapes[h].jFinal/*+ tamanhoi/2 */; j--, f++) { //f controla peso
                    if(pesos[tamanho_menor][tamanho_maior][f] != 0) {
                        for(int p = 0; p < CANAIS_IMAGEM; p++)
                        final->dados[p][corneri][j] = (pesos[tamanho_menor][tamanho_maior][f] * img->dados[p][coordenada_pixel_misturar][j]) 
                        + ((1-pesos[tamanho_menor][tamanho_maior][f]) * img->dados[p][corneri][j]);
                    }
                }
            }
            else {
