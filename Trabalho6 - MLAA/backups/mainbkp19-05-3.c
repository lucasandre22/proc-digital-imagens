...
void computaBordaColorida(Imagem* img, Imagem* bordas, Imagem* bordas_falsas) {
    ...
    if(img->n_canais == 3) {
        copia = criaImagem(img->largura, img->altura, 1);
...
void calculaPesos(float pesos[][5][5]) { //acessa linha, coluna
    ...
    pesos[0][4][0] = 3.f/7.f;;//
    pesos[0][4][1] = 9.f/24.f;//
    pesos[0][4][2] = 4.f/24.f;
    ...
    //cinco pixels na maior perna
    pesos[1][4][0] = 4.f/7.f;;//
    pesos[1][4][1] = 11.f/24.f;//
    pesos[1][4][2] = 7.f/24.f;
...
int main ()
{
    ...
    tempo_final = (((double)(clock() - tempo_inicio)) / CLK_TCK);
