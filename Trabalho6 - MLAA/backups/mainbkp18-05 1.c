...
void merge(Imagem* img, Imagem* bordas, Imagem* bordas_falsas, SHAPES* shapes) {
    ...
    for(int h = 0; h < shapes->quantidade; h++) {
        ...
        tamanhoi = shapes->l_shapes[h].iFinal > corneri ? shapes->l_shapes[h].iFinal - corneri : corneri - shapes->l_shapes[h].iFinal;
        tamanhoj = shapes->l_shapes[h].jFinal > cornerj ? shapes->l_shapes[h].jFinal - cornerj : cornerj - shapes->l_shapes[h].jFinal;
        ...
        if(tamanhoi == tamanhoj) { //quando é um só píxel, tratamento diferente
            int pixels_a_mergir;
            int direita = (vetor[shapes->l_shapes[h].borda_falsa]->dados[0][corneri][cornerj+1] == PONTA_FALSA);
            int esquerda = (vetor[shapes->l_shapes[h].borda_falsa]->dados[0][corneri][cornerj-1] == PONTA_FALSA);
            int baixo = (vetor[shapes->l_shapes[h].borda_falsa]->dados[0][corneri-1][cornerj] == PONTA_FALSA);
            int cima = (vetor[shapes->l_shapes[h].borda_falsa]->dados[0][corneri+1][cornerj] == PONTA_FALSA);
            float aux = pesos[0][0][1] + tamanho_maior * pesos[0][0][1];
            if(tamanhoi == 0) {
